<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column :label="$t('table.sendername')" prop="sendername" width="200px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.sendername }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.content')" min-width="100px">
        <template slot-scope="{row}">
          {{ row.content }}
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.receivename')" prop="receivename" width="200px" align="center">
        <template slot-scope="{row}">
      ...
      <el-table-column :label="$t('table.type')" class-name="status-col" width="110">
        <template slot-scope="{row}">
          <el-tag :type="row.type | statusFilter">
            ...
            <span v-if="row.type == 0">系统消息</span>
          </el-tag>
        </template>
      </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      isOptions: [{ label: '管理员消息', key: '1' }, { label: '系统消息', key: '0' }],
  ...
  methods: {
    ...
    handleDelete(id, index) {
      this.deleteId.id = id
      deleteMessage(this.deleteId).then(() => {
        this.$notify({
          title: '成功',
          message: '删除成功',
          type: 'success',
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['id', 'sendername', 'content', 'receivename', 'type', 'createtime']
        const filterVal = ['id', 'sendername', 'content', 'receivename', 'type', 'createtime']
