<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column :label="$t('table.modular')" min-width="100px">
        <template slot-scope="{row}">
...
<script>
import { fetchList, createModular, updateModular, deleteModular } from '@/api/modular'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        modular: [{ required: true, message: 'modular is required', trigger: 'blur' }]
      },
      props: {
        checkStrictly: true
      },
  ...
  methods: {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          createModular(this.temp).then(response => {
    ...
    handleDelete(id, index) {
      this.deleteId.id = id
      deleteModular(this.deleteId).then(response => {
        this.$notify({
          title: '成功',
          message: '删除成功',
          type: 'success',
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['id', 'modular', 'createtime', 'updatetime']
        const filterVal = ['id', 'modular', 'createtime', 'updatetime']
        const data = this.formatJson(filterVal)
        excel.export_json_to_excel({
          header: tHeader,
          data,
          filename: 'Modular_List'
        })
