<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.root" maxlength="20" :placeholder="$t('table.root')" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column :label="$t('table.root')" min-width="100px">
        <template slot-scope="{row}">
          <span class="link-type" @click="handleUpdate(row)">{{ row.root }}</span>
        </template>
      </el-table-column>
...
<script>
import { fetchList, createRoot, updateRoot, deleteRoot } from '@/api/root'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        root: [{ required: true, message: 'root is required', trigger: 'blur' }]
      },
  ...
  methods: {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['id', 'root', 'createtime', 'updatetime']
        const filterVal = ['id', 'root', 'createtime', 'updatetime']
