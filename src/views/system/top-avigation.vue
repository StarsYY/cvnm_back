...
<script>
import { fetchList, createNavigation, updateNavigation, deleteNavigation } from '@/api/navigation'
    ...
    return {
      ...
      rules: {
        ...
        jump: [
          ...
          { required: true, message: 'jump is required', trigger: 'blur' }
        ],
      },
  ...
  methods: {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createNavigation(this.temp).then(response => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateNavigation(tempData).then(() => {
    ...
    handleDelete(id, index) {
      this.deleteId.id = id
      deleteNavigation(this.deleteId).then(() => {
        this.$notify({
          title: '成功',
          message: '删除成功',
          type: 'success',
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['id', 'name', 'jump', 'createtime', 'updatetime']
        const filterVal = ['id', 'name', 'jump', 'createtime', 'updatetime']
        const data = this.formatJson(filterVal)
        excel.export_json_to_excel({
          header: tHeader,
          data,
          filename: 'Navigation_List'
        })
