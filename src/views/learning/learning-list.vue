<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column :label="$t('table.course')" prop="username" align="center">
        <template slot-scope="{row}">
          <span>{{ row.name }}</span>
        </template>
      </el-table-column>
...
<script>
import { fetchList, deleteLearning } from '@/api/learning'
...
export default {
  ...
  methods: {
    ...
    handleDelete(id, index) {
      this.deleteId.id = id
      deleteLearning(this.deleteId).then(() => {
        this.$notify({
          title: '成功',
          message: '删除成功',
          type: 'success',
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['id', 'username', 'name', 'createtime']
        const filterVal = ['id', 'username', 'name', 'createtime']
