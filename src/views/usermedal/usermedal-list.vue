<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-input v-model="listQuery.name" maxlength="10" :placeholder="$t('table.medal')" clearable style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column :label="$t('table.medal')" prop="username" align="center">
        <template slot-scope="{row}">
          <span>{{ row.medal }}</span>
        </template>
      </el-table-column>
...
<script>
import { fetchList, deleteUserMedal } from '@/api/userMedal'
...
export default {
  ...
  methods: {
    ...
    handleDelete(id, index) {
      this.deleteId.id = id
      deleteUserMedal(this.deleteId).then(() => {
        this.$notify({
          title: '成功',
          message: '删除成功',
          type: 'success',
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['id', 'username', 'medal', 'createtime']
        const filterVal = ['id', 'username', 'medal', 'createtime']
        const data = this.formatJson(filterVal)
        excel.export_json_to_excel({
          header: tHeader,
          data,
          filename: 'UserMedal-list'
        })
