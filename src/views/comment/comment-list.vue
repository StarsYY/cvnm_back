<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.comment" :placeholder="$t('table.comment')" clearable style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
      <el-input v-model="listQuery.article" maxlength="120" :placeholder="$t('table.article')" clearable style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
      <el-input v-model="listQuery.nickname" maxlength="20" :placeholder="$t('table.username')" clearable style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column :label="$t('table.article')" min-width="100px">
        <template slot-scope="{row}">
          {{ row.article }}
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.reviewname')" prop="reviewname" width="200px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.reviewName }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.comid')" prop="comid" width="80px" align="center">
        <template slot-scope="{row}">
          <span v-if="row.comid > 0">{{ row.comid }}</span>
        </template>
      </el-table-column>
...
<script>
import { fetchList, changeStatus, deleteComment } from '@/api/comment'
...
export default {
  ...
  methods: {
    ...
    handleDelete(id) {
      this.deleteId.id = id
      deleteComment(this.deleteId).then(response => {
        this.$notify({
          title: '成功',
          message: '删除成功',
          type: 'success',
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['id', 'comment', 'username', 'article', 'reviewName', 'createtime', 'status']
        const filterVal = ['id', 'comment', 'username', 'article', 'reviewName', 'createtime', 'status']
